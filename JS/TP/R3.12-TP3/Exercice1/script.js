...
V.renderCharacters = function( data ){
    ...
    for(let i =0; i < data.length; i++){
        allHtmlItem += V.formatOneItem(data[i].nom);
    }
    let ul = document.querySelector('.sidenav ul');
    ul.innerHTML = allHtmlItem;
}
...
C.handler_clickOnFilter = function(ev){
    ...
    let filtre = ev.target.dataset.filter;
    if (filtre != undefined){
        V.renderCharacters(M.filterCharacters(filtre));
    }
